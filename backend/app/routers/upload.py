...
async def serve_file(filename: str):
    ...
    try:
        ...
        original_filename = upload_record["original_filename"] if upload_record else filename
...
async def list_user_uploads(
    ...
    try:
        ...
        cursor = uploads_collection.find(filter_query).sort("created_at", -1).skip(skip).limit(limit)
...
async def delete_upload(
    ...
    try:
        ...
        # 권한 확인 (업로더 또는 관리자만)
        if upload_record["uploader_id"] != current_user["user_id"] and current_user.get("role") != "admin":
            raise HTTPException(
