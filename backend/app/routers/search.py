...
from ..routers import docs, blog, forum
    ...
    try:
        ...
        if 'docs' in search_types:
            try:
                ...
                docs_cursor = docs_collection.find(docs_query).limit(limit // len(search_types))
                async for doc in docs_cursor:
                    result = SearchResult(
                        ...
                        excerpt=doc.get("metadata", {}).get("description") or doc.get("excerpt"),
                        tags=doc.get("tags", []),
        ...
        if 'blog' in search_types:
            try:
                ...
                for post in mock_blog_posts:
                    if (q.lower() in post['title'].lower() or 
                        q.lower() in post['content'].lower() or
                        ...
                        if len([r for r in results if r.type == 'blog']) >= limit // len(search_types):
                            break
            except Exception as e:
            ...
            try:
                ...
                for post in mock_forum_posts:
                    if (q.lower() in post['title'].lower() or 
                        ...
                        if len([r for r in results if r.type == 'forum']) >= limit // len(search_types):
                            break
            except Exception as e:
...
async def get_popular_searches(
    limit: int = Query(10, description="인기 검색어 수")
):
    """인기 검색어 조회"""
    try:
