...
class OIDCAuth:
    ...
    async def handle_callback(self, code: str, state: str) -> Dict[str, Any]:
        ...
        session_data = self.sessions.get(state)

        # State 재사용 방지 (첫 번째 성공적인 처리 후 제거)
        if session_data.get("used"):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="State parameter already used",
            )

        # State를 사용됨으로 표시
        self.sessions[state]["used"] = True
        ...
        try:
            ...
            self.sessions.pop(state, None)
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=f"OAuth error: {str(e)}"
            )
        except Exception as e:
            # 실패한 경우에도 state 정리
            self.sessions.pop(state, None)
    ...
    async def _handle_dummy_token_exchange(
        ...
        async with httpx.AsyncClient() as client:
            token_endpoint = settings.OIDC_DISCOVERY_URL.replace(
                "/.well-known/openid_configuration", "/token"
            )
